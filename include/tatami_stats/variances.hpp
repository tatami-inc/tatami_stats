 ...
 *
 * @param range A `tatami::SparseRange` object specifying the number and values of all non-zero indices.
...
std::pair<Output_, Output_> compute_direct(const tatami::SparseRange<Value_, Index_>& range, size_t n) {
    if (n < 1) {
        return both_NaN<Output_>();
    }
...
void compute_running(const tatami::SparseRange<Value_, Index_>& range, Output_* means, Output_* vars, Nonzero_* nonzeros, int& count, bool skip_zeros = true, Index_ subtract = 0) {
    ++count;
    for (Index_ j = 0; j < range.number; ++j) {
 ...
 * @cond
 */
namespace variance_internal {

template<bool row_, typename Value_, typename Index_, typename Output_>
void dimension_variances(const tatami::Matrix<Value_, Index_>* p, Output_* output, int threads) {
    ...
    if (p->sparse()) {
        if (direct) {
            ...
            tatami::parallelize([&](size_t, Index_ s, Index_ l) {
                auto ext = tatami::consecutive_extractor<true>(p, row_, s, l);
                std::vector<Value_> vbuffer(otherdim);
                for (Index_ x = 0; x < l; ++x) {
                    auto out = ext->fetch(vbuffer.data(), NULL);
                    output[x + s] = compute_direct<Output_>(out, otherdim).second;
                }
            }, dim, threads);
            ...
            tatami::parallelize([&](size_t, Index_ s, Index_ l) {
                ...
                for (Index_ x = 0; x < otherdim; ++x) {
                    ...
                    compute_running(out, running_means.data(), running_vars, running_nzeros.data(), counter, /* skip_zeros = */ true, /* subtract = */ s);
                }
                finish_running(l, running_means.data(), running_vars, running_nzeros.data(), counter);
            }, dim, threads);
        }
        ...
        if (direct) {
            ...
            tatami::parallelize([&](size_t, Index_ s, Index_ l) {
                ...
                for (Index_ x = 0; x < otherdim; ++x) {
                    auto out = ext->fetch(buffer.data());
                    compute_running(out, l, running_means.data(), running_vars, counter);
                }
                finish_running(l, running_means.data(), running_vars, counter);
            }, dim, threads);
        }
    }
}
...
void column_variances(const tatami::Matrix<Value_, Index_>* p, Output_* output, int threads = 1) {
    variance_internal::dimension_variances<false>(p, output, threads);
    return;
}
...
void row_variances(const tatami::Matrix<Value_, Index_>* p, Output_* output, int threads = 1) {
    variance_internal::dimension_variances<true>(p, output, threads);
    return;
}
